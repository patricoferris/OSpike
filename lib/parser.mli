...
module HashableBuffer : sig 
  include Base__.Hashtbl_intf.Key with type t = Riscv.t Buff.t
  val copy : t -> t 
end

module MakeHashableBuffer (C : RiscvComparator) : sig 
  include Base__.Hashtbl_intf.Key with type t = Riscv.t Buff.t
  val copy : t -> t 
end
...
val from_stdin : Parser_options.t -> (HashableBuffer.t, int) Core.Hashtbl.t
...
val print_sorted : out_channel -> (HashableBuffer.t, int) Core.Hashtbl.t -> Parser_options.t -> unit 
